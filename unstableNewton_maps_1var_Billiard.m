function [lambda_new, P_new] = unstableNewton_maps_1var_Billiard(...
    coef, lambda0, P0, NFFT)
% UNSTABLENEWTON_MAPS_1VAR_BILLIARD_PER2 - Solve unstable manifold equation
%
% SYNTAX:
%   [lambda_new, P_new] = unstableNewton_maps_1var_Billiard_Per2(...
...
%
% DESCRIPTION:
%   Solves P(theta) = F(P(mu*theta)) with normalization constraint
%   using Newton iteration, where mu = 1/lambda. Includes adaptive
%   stopping and rollback on error increase.
...
if abs(lambda0) < 1
    error('Initial eigenvalue must be > 1 for unstable manifold');
end

% Initialize state (mu = 1/lambda for unstable case)
mu = 1 / lambda0;
P = P0;

% Initial residual
[Phi0, PhiP, DTP_mu] = parmU_FEMap_Billiard(mu, P, scale, coef, NFFT);
...
for m = 1:10
    ...
    % Assemble full Jacobian (note: different from stable case)
    partialPhi1 = -DTP_matrix * P_prime_mu_theta;
    DPhi_matrix = [0, partialPhi0; partialPhi1, Id_matrix - DTP_matrix * Mu_matrix];
